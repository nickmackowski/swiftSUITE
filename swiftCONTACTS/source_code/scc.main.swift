...
class ContactsDebugLogger {
    ...
    /// beside the binary). Created on first access if it doesn't exist yet. contacts.json and
    ...
    static let logURL: URL = {
        ...
        return logsDir.appendingPathComponent("swiftcontacts_\(ts).log")
    }()
...
struct Contact: Codable {
    ...
    var tags: [String] = []
    // MM/DD, no year — deliberately plaintext too, same tier as the above, so swiftCALENDAR can
    // compute recurring birthday events without needing the master password. Full dob (with
    // year) stays inside encryptedDetails below, untouched. Optional (not excluded from
    // CodingKeys) since this genuinely needs to round-trip to disk — Swift's synthesized
    // Decodable treats a missing key as nil for Optional properties automatically, so contacts
    // without a birthday set just decode as nil, no special handling needed.
...
class ContactsManager {
    ...
    private func unlockOrCreateContacts() {
        guard let sessionKey = readCoreSessionKey(appID: "swiftCONTACTS") else {
        ...
        if let contactsFile = try? JSONDecoder().decode(ContactsFile.self, from: data) {
            if let decryptedCanary = try? ContactsCrypto.decrypt(contactsFile.canary, key: sessionKey),
            ...
            // Decodes as a ContactsFile but this session key can't open it — it was protected
        ...
        if let legacyContacts = try? JSONDecoder().decode([LegacyContact].self, from: data) {
            migrateLegacyContacts(legacyContacts, sessionKey: sessionKey)
            return
        }
    ...
    private func migrateLegacyContacts(_ legacyContacts: [LegacyContact], sessionKey: SymmetricKey) {
        print("\u{001B}[2J\u{001B}[1;1H", terminator: "")
        printStandardHeader()
        ...
        print(" protected by your swiftCORE login.\n")
        ...
        for legacy in legacyContacts {
            ...
            let encrypted = (try? ContactsCrypto.encrypt(detailsJSON, key: sessionKey)) ?? ""
    ...
    /// Parses the leading "M/D" out of a loose date string (works on both a bare "M/D" or
    /// "MM/DD" entry and a full dob like "7/4/1969" or "07/04/69" — only the first two
    /// components matter) and returns it zero-padded as "MM/DD". Returns nil if it doesn't look
    /// like a parseable date at all, so callers can fall back to the raw input rather than
    /// silently discarding something that was typed.
    private func parseAndPadMonthDay(_ input: String) -> String? {
    ...
    private func printStandardHeader() {
        ...
        let innerWidth = 118
        let titleText = "swiftCONTACTS v3.01.07.31c"  // plain for layout; colored below
        ...
        // "swift" plain white; "CONTACTS" solid orange (shared with swiftADMIN — separate
        // compiled binaries, never displayed together, so no collision risk; see suite color map)
        ...
        titleLineChars[sidePadding + 5]  = "\u{001B}[1;38;5;208mC\u{001B}[0m"
        titleLineChars[sidePadding + 6]  = "\u{001B}[1;38;5;208mO\u{001B}[0m"
        titleLineChars[sidePadding + 7]  = "\u{001B}[1;38;5;208mN\u{001B}[0m"
        titleLineChars[sidePadding + 8]  = "\u{001B}[1;38;5;208mT\u{001B}[0m"
        titleLineChars[sidePadding + 9]  = "\u{001B}[1;38;5;208mA\u{001B}[0m"
        titleLineChars[sidePadding + 10] = "\u{001B}[1;38;5;208mC\u{001B}[0m"
        titleLineChars[sidePadding + 11] = "\u{001B}[1;38;5;208mT\u{001B}[0m"
        titleLineChars[sidePadding + 12] = "\u{001B}[1;38;5;208mS\u{001B}[0m"
    ...
    private func colorizeFooterKeys(_ line: String) -> String {
        ...
        let colored = segments.map { segment -> String in
            if let bracketRange = segment.range(of: "]"), segment.trimmingCharacters(in: .whitespaces).hasPrefix("[") {
                ...
                return "\u{001B}[1;38;5;208m\(keyPart)\u{001B}[0m\(rest)"
            }
        ...
        while true {
            ...
            case .other(let ch):
                ...
                if lower == "a" {
                    ...
                    // Nav footer — [A] is Add Contact, [T] is current app (no-op, self-omitted
                    // from navMap — previously included itself, which relaunched the app)
    ...
    func showSearchScreen() {
        printStandardHeader()
        let title = ">>> swiftCONTACTS SEARCH <<<"
    ...
    func showResultsScreen(results: [Contact], title: String) {
        ...
        while true {
            ...
            for (idx, contact) in sorted.enumerated() {
                ...
                let content = "[\(idx + 1)]. \(contact.displayName)\(companySuffix)"
                if isSelected {
                    let full = " -> " + content
            ...
            case .other(let ch):
                ...
                // fixed in swiftSTOCKS/swiftVAULT's search-results screens. [T] is current app
                // (no-op, self-omitted).
                let lower = Character(ch.lowercased())
                let navMap: [Character: String] = [
                    "c": "swiftCALENDAR",
    ...
    func showViewContactScreen(index: Int) {
        ...
        while true {
            ...
            if let bday = contact.birthdayMonthDay, !bday.isEmpty { infoRow("Calendar B-day", bday) }
            ...
            case .other(let ch):
                ...
                if lower == "e" {
                    ...
                    // Nav footer. Two fixes here: (1) simplified to omit "t" from the map
                    // entirely, matching how swiftSTOCKS/swiftVAULT correctly self-omit their
                    // own letter, rather than the previous awkward "target != swiftCONTACTS"
                    // guard. (2) Added the missing disableRawMode()/enableRawMode() bracketing
                    // around navigateToApp() — same bug found and fixed in swiftVAULT's
    ...
    func showAddContactScreen() {
        printStandardHeader()
        let title = ">>> swiftCONTACTS ADD CONTACT <<<"
        let pad = max(0, (118 - title.count) / 2)
        print(String(repeating: " ", count: pad) + title)
        print(String(repeating: "─", count: 120))
        ...
        details.dob = getStringInput(prompt: " Date of Birth (MM/DD/YY): ") ?? ""
        // Plaintext, separate from encrypted dob above — see Contact struct comment.
        // Auto-suggested from the dob just entered above (Enter to accept, or type to
        // override/clear) — zero-padded automatically either way, so leading zeros are never
        // something the user has to remember to type.
        let suggestedBday = parseAndPadMonthDay(details.dob)
        let bdayPrompt = suggestedBday != nil
            ? " Birthday for calendar (MM/DD) [\(suggestedBday!)] (Enter to accept): "
            : " Birthday for calendar (MM/DD, optional): "
        let birthdayInput = getStringInput(prompt: bdayPrompt, defaultValue: suggestedBday)
        let birthdayMonthDay: String? = {
            guard let raw = birthdayInput, !raw.isEmpty else { return nil }
            return parseAndPadMonthDay(raw) ?? raw
        }()
    ...
    func showEditContactScreen(index: Int) {
        ...
        printStandardHeader()
        let title = ">>> swiftCONTACTS EDIT CONTACT <<<"
        let pad = max(0, (118 - title.count) / 2)
        print(String(repeating: " ", count: pad) + title)
        print(String(repeating: "─", count: 120))
        ...
        // Plaintext, separate from encrypted dob above — see Contact struct comment. Keeps the
        // existing value on Enter (same convention as every other field here); if there isn't
        // one yet, suggests deriving it from dob instead — either way, zero-padded automatically.
        let currentBday = contact.birthdayMonthDay
        let bdaySuggestion = currentBday ?? parseAndPadMonthDay(details.dob)
        let bdayEditPrompt = bdaySuggestion != nil
            ? " Birthday for calendar (MM/DD) [\(bdaySuggestion!)]: "
            : " Birthday for calendar (MM/DD, optional): "
        if let val = getStringInput(prompt: bdayEditPrompt, defaultValue: bdaySuggestion), !val.isEmpty {
            contact.birthdayMonthDay = parseAndPadMonthDay(val) ?? val
        }
    ...
    func showDbUtilitiesMenu() {
        ...
        while true {
            print("\u{001B}[2J\u{001B}[1;1H", terminator: "")
            printStandardHeader()
            let title = ">>> swiftCONTACTS UTILITIES <<<"
            let pad = max(0, (118 - title.count) / 2)
            print(String(repeating: " ", count: pad) + title)
    ...
    private func backupDatabase() {
        ...
        do {
            ...
            print(" by your swiftCORE login. Name/email were already stored in plain text.)")
    ...
    private func restoreDatabase() {
        ...
        do {
            ...
            while true {
                print("\u{001B}[2J\u{001B}[1;1H", terminator: "")
                printStandardHeader()
                let title = ">>> swiftCONTACTS BACKUPS <<<"
                let pad = max(0, (118 - title.count) / 2)
                print(String(repeating: " ", count: pad) + title)
    ...
    private func triggerDatabaseRestore(file: URL) throws {
        ...
        // Try swiftCORE's unified-auth session key — this is how contacts is unlocked day-to-day,
        ...
        if let sessionKey = readCoreSessionKey(appID: "swiftCONTACTS"),
           ...
           decryptedCanary == ContactsCrypto.canaryPlaintext {
            ...
            self.lastBackupTimestamp = contactsFile.lastBackupTimestamp
            recomputeContactCaches()
            ContactsDebugLogger.log("Contacts restored from \(file.lastPathComponent) via unified auth session", category: "CONTACTS")
    ...
    func exportToCSV() {
        ...
        var csvString = "FirstName,LastName,DOB,BirthdayMonthDay,Spouse,Phone,PersonalEmail,Street,City,State,Zip,Company,WorkPhone,WorkEmail,Tags\n"
        ...
        for c in contacts {
            ...
            let fields = [
                c.firstName, c.lastName, details.dob, c.birthdayMonthDay ?? "", details.spouseName,
    ...
    func importFromCSV() {
        ...
        do {
            ...
            for row in rows where !row.isEmpty {
                ...
                details.spouseName = fields[4]
                details.phone = fields[5]
                details.street = fields[7]
                details.city = fields[8]
                details.state = fields[9]
                details.zip = fields[10]
                details.company = fields[11]
                details.workPhone = fields[12]
                ...
                let tags = fields.count >= 15 ? parseTagsFromString(fields[14]) : []
                let birthdayMonthDay = fields[3].isEmpty ? nil : fields[3]
                
                let newContact = Contact(
                    firstName: fields[0],
                    lastName: fields[1],
                    personalEmail: fields[6],
                    workEmail: fields[13],
                    tags: tags,
                    birthdayMonthDay: birthdayMonthDay,
                    encryptedDetails: encrypted,
