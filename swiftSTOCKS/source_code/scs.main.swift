...
class StocksDebugLogger {
    ...
    /// beside the binary). Created on first access if it doesn't exist yet. stocks.json and
    ...
    static let logURL: URL = {
        ...
        return logsDir.appendingPathComponent("swiftstocks_\(ts).log")
    }()
...
class StockKeyboardReader {
    ...
    /// waiting for input. Passing nil for timeoutSeconds falls back to a plain blocking read,
    /// identical to calling readKey() directly. Mirrors the same waitForKey() already built and
    /// proven in swiftMAIL and swiftNOTES, for consistency across the suite.
    func waitForKey(timeoutSeconds: Double?) -> StockKeyPress? {
...
class StocksManager {
    ...
    // (and the shared `updatedCount` counter) from those completion handlers, and now also
    // guards isSyncing/syncJustFinished (logically part of the same sync state) and every read
    // of `stocks` from showWorkspace()'s render loop, since that loop can now run concurrently
    // with a background sync for the first time (previously the app always blocked on
    // dispatchGroup.wait() before any rendering happened at all, so this race never existed).
    private let stocksMutationLock = NSLock()
    private var isSyncing = false
    private var syncJustFinished = false
    private var statusSpinnerFrame = 0 // advances each printPortfolioStatusBlock() call while syncing
    ...
    init() {
        ...
        // Non-blocking on launch — the workspace renders immediately with whatever prices were
        // saved from the last session, then this fills in live prices in the background.
        // Previously this called syncLiveQuotes(isSilent: false) directly, which ends in a
        // blocking dispatchGroup.wait() — the exact same launch-blocking bug already fixed in
        // swiftMAIL and swiftNOTES, just not yet applied here.
        stocksMutationLock.lock()
        isSyncing = true
        stocksMutationLock.unlock()
        syncLiveQuotes(isSilent: true, blocking: false)
    }
    ...
    /// Set `blocking: false` to return immediately and finish the sync in the background instead
    /// of waiting for every fetch — used on launch so the workspace can render right away.
    /// Manual [R] refresh keeps the default blocking: true, matching its existing behavior
    ...
    func syncLiveQuotes(isSilent: Bool = false, blocking: Bool = true) {
        if stocks.isEmpty {
            ...
            if !blocking {
                // init() already set isSyncing = true before calling this — without this, an
                // empty portfolio would leave that flag stuck true forever, since nothing else
                // would ever clear it.
                stocksMutationLock.lock()
                isSyncing = false
        ...
        if blocking {
            ...
            // Fires once every task's completion handler has run — no blocking wait on this
            // thread. Runs finishSync() first (same save/log/timestamp work as the blocking
            // path), then flips the flags showWorkspace()'s render loop is polling for.
            dispatchGroup.notify(queue: DispatchQueue.global()) { [weak self] in
                guard let self = self else { return }
                finishSync()
                self.stocksMutationLock.lock()
                self.isSyncing = false
                self.syncJustFinished = true
                self.stocksMutationLock.unlock()
            }
        }
    }
    ...
    private func printStandardHeader(context: String? = nil) {
        ...
        let titleText = "swiftSTOCKS v3.01.07.29c"  // plain for layout; colored below
        ...
        // "swift" plain white; "STOCKS" solid green
        ...
        titleLineChars[sidePadding + 5]  = "\u{001B}[1;32mS\u{001B}[0m"
        titleLineChars[sidePadding + 6]  = "\u{001B}[1;32mT\u{001B}[0m"
        titleLineChars[sidePadding + 7]  = "\u{001B}[1;32mO\u{001B}[0m"
        titleLineChars[sidePadding + 8]  = "\u{001B}[1;32mC\u{001B}[0m"
        titleLineChars[sidePadding + 9]  = "\u{001B}[1;32mK\u{001B}[0m"
        titleLineChars[sidePadding + 10] = "\u{001B}[1;32mS\u{001B}[0m"
    ...
    private func printPortfolioStatusBlock(stillSyncing: Bool) {
        ...
        // Same fixed-slot status-line-swap approach already built for swiftMAIL and swiftNOTES:
        // this line always shows SOMETHING here, just different content depending on state,
        // rather than an extra line appearing/disappearing above the rest of the layout.
        if stillSyncing {
            let spinnerFrames = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
            let frame = spinnerFrames[statusSpinnerFrame % spinnerFrames.count]
            statusSpinnerFrame += 1
            print(" \u{001B}[1;36mStatus: [ \(frame) ] Checking live quotes...\u{001B}[0m")
        } else {
    ...
    func showWorkspace() {
        ...
        while true {
            ...
            if syncJustFinished { syncJustFinished = false }
            // Snapshot under the lock — showWorkspace() can now run concurrently with a
            // background sync for the first time (previously the app always blocked until
            // sync fully finished before any rendering happened, so this race never existed).
            // Array is a value type, so this copy is safe to read/sort/display outside the
            // lock once taken.
            ...
            print("\u{001B}[2J\u{001B}[1;1H", terminator: "")
            printStandardHeader()
            printPortfolioStatusBlock(stillSyncing: stillSyncing)
            ...
            guard let key = keyboard.waitForKey(timeoutSeconds: stillSyncing ? 0.2 : nil) else {
                continue // timed out while the background sync is still running — loop back and repaint
            }
        ...
        while true {
            ...
            case .other(let ch):
                if ch == "r" || ch == "R" {
                    ...
                    // Nav footer — direct app switching. Was missing here entirely (bug), same
                    // issue as showViewStockScreen. [S] stays as Stocks (current app, no-op).
                    let lower = Character(ch.lowercased())
                    let navMap: [Character: String] = [
                        "t": "swiftCONTACTS", "c": "swiftCALENDAR",
                        "m": "swiftMAIL",     "n": "swiftNOTES",
        ...
        while true {
            ...
            case .other(let ch):
                ...
                if lower == "e" {
                    ...
                    // Nav footer — direct app switching. Was missing here entirely (bug), which
                    // is why the nav keys worked from the main workspace grid but not from this
                    // detail screen. [S] stays as Stocks (current app, no-op).
                    let navMap: [Character: String] = [
                        "t": "swiftCONTACTS", "c": "swiftCALENDAR",
                        "m": "swiftMAIL",     "n": "swiftNOTES",
                        "v": "swiftVAULT"
    ...
    func showUtilitiesMenu() {
        ...
        while true {
            ...
            // numbered-selection (1-X) only, no app-switching nav footer. Confirmed against
            // every other app's own utilities menu, which are all footer-less.
