...
class CoreDebugLogger {
    ...
    /// beside the binary). Created on first access if it doesn't exist yet. Data files
    /// (credentials, sessions, etc.) are unaffected and stay exactly where they were.
    static let logsDir: URL = {
        let execPath = CommandLine.arguments[0]
        let dir = URL(fileURLWithPath: execPath).deletingLastPathComponent()
    ...
    static let logURL: URL = {
        ...
        return logsDir.appendingPathComponent("swiftcore_\(ts).log")
    }()
...
func printHeader(snapshot: SystemSnapshot, username: String) {
    ...
    let titleText = "swiftCORE v3.01.07.29c"  // plain for layout; colored below
    ...
    // REVERSED from the original scheme: "swift" is now plain white, "CORE" gets one solid
    // accent color (cyan) instead of swift's old per-letter rainbow. Matches the same swap
    // applied to swiftCoreBannerColored() below. Still one escape-wrapped slot per character
    // (not one slot for the whole word) to stay consistent with how titleChars already works.
    titleChars[sidePad + 0] = "\u{001B}[1;97ms\u{001B}[0m"
    titleChars[sidePad + 1] = "\u{001B}[1;97mw\u{001B}[0m"
    titleChars[sidePad + 2] = "\u{001B}[1;97mi\u{001B}[0m"
    titleChars[sidePad + 3] = "\u{001B}[1;97mf\u{001B}[0m"
    titleChars[sidePad + 4] = "\u{001B}[1;97mt\u{001B}[0m"
    titleChars[sidePad + 5] = "\u{001B}[1;36mC\u{001B}[0m"
    titleChars[sidePad + 6] = "\u{001B}[1;36mO\u{001B}[0m"
    titleChars[sidePad + 7] = "\u{001B}[1;36mR\u{001B}[0m"
    titleChars[sidePad + 8] = "\u{001B}[1;36mE\u{001B}[0m"
...
/// All 9 letters of "swiftCORE" get their own color, drawn from the same suite-wide palette
/// used for app identities (plus Yellow and Silver, which don't collide with anything in this
/// particular banner context the way they did elsewhere — Yellow only clashed with the
/// adjacent "User:" line in the compact HEADER, not relevant in this banner box; Silver only
/// clashed with a plain-white "swift" that no longer exists here since every letter is colored
/// now). C/O/R/E keep the same 4 colors as before (Cyan/Orange/Green/Teal — swiftCORE's own
/// launcher-for-everyone callback); s/w/i/f/t fill out the remaining 5 from the palette.
/// Shared by the login screen, the post-login app-select screen, and the first-launch setup
/// screen so all three stay in sync. The compact top header/telemetry line is untouched by
/// this — it keeps plain white "swift" + solid cyan "CORE", per prior instruction.
func swiftCoreBannerColored() -> String {
    let sMagenta = "\u{001B}[1;35ms\u{001B}[0m"
    let wYellow  = "\u{001B}[1;33mw\u{001B}[0m"
    let iPink    = "\u{001B}[1;38;5;211mi\u{001B}[0m"
    let fMint    = "\u{001B}[1;38;5;121mf\u{001B}[0m"
    let tSilver  = "\u{001B}[1;38;5;252mt\u{001B}[0m"
    let cC = "\u{001B}[1;36mC\u{001B}[0m"
    let oO = "\u{001B}[1;38;5;208mO\u{001B}[0m"
    let gR = "\u{001B}[1;32mR\u{001B}[0m"
    let tE = "\u{001B}[1;38;5;30mE\u{001B}[0m"
    return sMagenta + " " + wYellow + " " + iPink + " " + fMint + " " + tSilver + " " + cC + " " + oO + " " + gR + " " + tE
}
...
func printLoginBox(lastLoginStr: String? = nil, message: String? = nil, messageIsError: Bool = false) {
    ...
    // s w i f t C O R E — all 9 letters colored, full palette rainbow, centered
