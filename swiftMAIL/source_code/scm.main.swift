...
enum AccountType: String, Codable, CaseIterable {
    ...
    // Short form for compact list display (e.g. "[Yahoo]" in Account Setup) — displayName is
    // too long to use there without wrapping awkwardly.
    ...
    // Best-known folder-naming convention per provider — used as the initial guess when an
    // account is first created, and as the pre-filled default for .custom's manual folder-path
    // entry. For known (non-.custom) providers, showEditSlotScreen() now runs real IMAP
    // discovery (RFC 6154 SPECIAL-USE, IMAPSession.discoverSpecialUseFolders()) right after the
    // account is saved, which overwrites these guesses with the server's actual folder names —
    // this table is just the fallback for when discovery itself can't run or comes up empty.
    ...
    // Gmail's SMTP server auto-saves a copy of anything sent through it (authenticated as that
    // Gmail account) into Sent Mail — well-documented, stable behavior, not something that
    // varies by client or needs runtime detection. This is specifically what caused the
    // duplicate-Sent-mail bug: appendMessageToSentFolder() used to run unconditionally after
    // every send, creating a second copy on top of the one Gmail already made server-side.
    // Every other provider here (Yahoo, iCloud, Fastmail, custom/generic SMTP) does NOT do this
    // — a plain SMTP server has no concept of "Sent," so the explicit IMAP APPEND is the only
    // way a copy ends up there at all for those.
    var autoSavesSentServerSide: Bool {
        switch self {
...
class MailDebugLogger {
    ...
    static let logURL: URL = {
        ...
        return logsDir.appendingPathComponent("swiftmail_\(ts).log")
    }()
...
class MailKeyboardReader {
    ...
    /// Same as readCanonicalLine(), but returns nil if the user presses ESC instead of typing
    /// something and hitting Enter. Kept as a separate method rather than changing
    /// readCanonicalLine() itself (which has 15 other call sites elsewhere in this file that
    /// don't need — and shouldn't have to start handling — an Optional return). Used in the
    /// account setup flow so there's a real way to back out of a multi-step form instead of
    /// being stuck until Ctrl+C.
    func readCanonicalLineOrEscape() -> String? {
        var inputString = ""
        var buffer = [UInt8](repeating: 0, count: 1)
    ...
    /// Returns nil if the user presses ESC — safe to change to Optional since this only has one
    /// call site (unlike readCanonicalLine's 15, which is why that one got a separate
    /// ESC-aware variant instead of changing its own signature).
...
final class IMAPSession {
    ...
    /// Parses a `* LIST (...) "delim" "name"` response line into its flags and mailbox name.
    /// Handles the common case every mainstream provider actually sends (quoted delimiter,
    /// quoted name, name may contain internal spaces like "Sent Items"). An unusual non-quoted
    /// or literal-encoded name is skipped rather than mis-parsed — the caller just won't find a
    /// match for that one folder and falls through to the next fallback.
    ///
    /// The returned name is re-wrapped in literal quote characters (e.g. `"[Gmail]/Sent Mail"`,
    /// quotes included in the string value) to match the convention every hardcoded serverPath
    /// in this file already uses (see AccountType.defaultSentPath etc., which bake quotes into
    /// the string the same way) — select(folder:) and append(folder:) interpolate whatever's
    /// stored directly into the IMAP command with no quoting of their own, so an unquoted name
    /// containing a space (like "Sent Mail") breaks the command server-side. This is a bug that
    /// was actually hit in testing: discovery correctly found "[Gmail]/Sent Mail" for a Workspace
    /// account, but without quotes, causing "could not select" even though the name was right.
    private func parseListLine(_ line: String) -> (flags: [String], name: String)? {
        guard line.hasPrefix("* LIST ") else { return nil }
        guard let flagsStart = line.firstIndex(of: "("), let flagsEnd = line.firstIndex(of: ")") else { return nil }
        let flags = line[line.index(after: flagsStart)..<flagsEnd].split(separator: " ").map(String.init)

        let afterFlags = line[line.index(after: flagsEnd)...]
        let quotedParts = afterFlags.split(separator: "\"").filter { !$0.trimmingCharacters(in: .whitespaces).isEmpty }
        guard let name = quotedParts.last else { return nil }
        return (flags, "\"\(name)\"")
    }

    /// RFC 6154 SPECIAL-USE discovery — asks the server directly which real folder is Sent/
    /// Trash/Junk/Drafts/Archive, rather than guessing a hardcoded name per provider (the
    /// approach that broke Yahoo, since its real folder names don't match the old guesses).
    /// Supported by Gmail, Yahoo, Outlook, iCloud, and Fastmail. Returns a dict keyed by the
    /// special-use attribute (e.g. "\\Sent") to the real mailbox name on this specific server.
    /// Empty dict if the server doesn't support SPECIAL-USE or the command fails — callers
    /// should fall back to discoverFoldersByHeuristic(), then to the hardcoded per-provider
    /// guess as the last resort.
    func discoverSpecialUseFolders() -> [String: String] {
        let result = command("LIST \"\" \"*\" RETURN (SPECIAL-USE)", timeout: 15.0)
        guard result.ok else {
            MailDebugLogger.log("SPECIAL-USE discovery: command failed — \(result.statusLine)", category: "FOLDER-DISCOVERY")
            return [:]
        }

        var found: [String: String] = [:]
        let specialUseAttrs = ["\\Sent", "\\Trash", "\\Junk", "\\Drafts", "\\Archive", "\\All"]
        for line in result.untagged {
            guard let (flags, name) = parseListLine(line) else { continue }
            for attr in specialUseAttrs where flags.contains(attr) {
                found[attr] = name
            }
        }
        MailDebugLogger.log("SPECIAL-USE discovery: \(result.untagged.count) LIST line(s), found=\(found), raw=\(result.untagged)", category: "FOLDER-DISCOVERY")
        return found
    }

    /// Fallback for servers without SPECIAL-USE support: lists every folder that actually
    /// exists and heuristically matches common naming patterns against that real list, instead
    /// of blindly guessing a single hardcoded string that may not exist on this server at all.
    /// Still strictly better than the old approach even when it can't find an exact match.
    func discoverFoldersByHeuristic() -> [String: String] {
        let result = command("LIST \"\" \"*\"", timeout: 15.0)
        guard result.ok else {
            MailDebugLogger.log("Heuristic discovery: command failed — \(result.statusLine)", category: "FOLDER-DISCOVERY")
            return [:]
        }

        var allNames: [String] = []
        for line in result.untagged {
            if let (_, name) = parseListLine(line) {
        ...
        func firstMatch(_ patterns: [String]) -> String? {
            for pattern in patterns {
                if let match = allNames.first(where: { $0.lowercased().contains(pattern) }) {
                    return match
                }
            }
            return nil
        ...
        var found: [String: String] = [:]
        if let sent = firstMatch(["sent"]) { found["\\Sent"] = sent }
        if let trash = firstMatch(["trash", "deleted"]) { found["\\Trash"] = trash }
        if let junk = firstMatch(["junk", "spam", "bulk"]) { found["\\Junk"] = junk }
        if let drafts = firstMatch(["draft"]) { found["\\Drafts"] = drafts }
        MailDebugLogger.log("Heuristic discovery: \(allNames.count) real folder(s) seen=\(allNames), found=\(found)", category: "FOLDER-DISCOVERY")
        return found
    }
...
class MailManager {
    ...
    var syncMessage = ""
    private var statusSpinnerFrame = 0 // advances each renderStatusBar() call while syncing
    ...
    private func printStandardHeader() {
        ...
        let innerWidth = 118
        let titleText = "swiftMAIL v3.01.08.01c"  // plain for layout; colored below
        ...
        // "swift" plain white; "MAIL" solid magenta
        ...
        titleLineChars[sidePadding + 5] = "\u{001B}[1;35mM\u{001B}[0m"
        titleLineChars[sidePadding + 6] = "\u{001B}[1;35mA\u{001B}[0m"
        titleLineChars[sidePadding + 7] = "\u{001B}[1;35mI\u{001B}[0m"
        titleLineChars[sidePadding + 8] = "\u{001B}[1;35mL\u{001B}[0m"
    ...
    private func renderStatusBar() {
        ...
        if syncingState {
            // Braille spinner instead of the original numeric fill-bar — matches swiftNOTES'
            // status bar style (user tried both, preferred this one; see the note to also update
            // this app's own future v3.0 pass to stay consistent). syncProgress is still updated
            // by the sync logic elsewhere but no longer drives this display.
            let frames = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
            let frame = frames[statusSpinnerFrame % frames.count]
            statusSpinnerFrame += 1
            print(" \u{001B}[1;36mStatus: [ \(frame) ] \(msg)\u{001B}[0m")
        ...
        if totalAccounts == 0 {
            ...
            case .charMenu(let char):
                ...
                if lowerChar == "a" {
                    ...
                    // Nav footer — was shown but not wired up (bug), same pattern found and
                    // fixed elsewhere in the suite. [M] is current app (self-omitted, no-op).
    ...
    func composePlaintextMail(replyTo: String? = nil, subjectPrefix: String? = nil) {
        ...
        if sendSucceeded {
            ...
            let spinnerMsg = currentAccount.type.autoSavesSentServerSide ? "Confirming send..." : "Archiving copy to Sent folder..."
            runWithSpinner(message: spinnerMsg) { [weak self] in
    ...
    private func appendMessageToSentFolder(to recipient: String, subject: String, body: String, account: EmailAccount) {
        ...
        // The actual fix for the duplicate-Sent-mail bug: skip the explicit APPEND entirely for
        // providers that already save a copy server-side on send (Gmail) — see
        // AccountType.autoSavesSentServerSide for why this is safe as a static per-provider flag
        // rather than something needing a runtime check.
        var appended = account.type.autoSavesSentServerSide
        if !appended {
            imapOperationLock.lock()
            if let session = imapSession(for: account) {
    ...
    func showAccountMatrix() {
        ...
        while true {
            ...
            if currentAccounts.isEmpty {
                print("               [ No active mail accounts. Press 'A' to add a new account. ]")
                for _ in 0..<10 { print("") }
            } else {
            ...
            case .charMenu(let char):
                let lowerChar = char.lowercased()
                if lowerChar == "a" {
                    // Was checking for "n" instead of "a" — didn't match the footer hint just
                    // below ("A: Add Account"), so pressing A silently did nothing.
    ...
    func showEditSlotScreen(accountIndex: Int) {
        ...
        // Lets ESC-to-cancel clean up correctly: a brand-new account (added via [A] Add Account
        // in showAccountMatrix, which appends + saves a placeholder before this screen even
        // starts) should have that placeholder removed on cancel, rather than leaving a stray
        // "[New Unconfigured Account]" entry sitting in the list. Editing an already-configured
        // account is unaffected either way — this function never writes accounts[accountIndex]
        // until it runs all the way to the end, so an early return here just leaves the existing
        // account exactly as it was before this screen opened.
        let isUnconfiguredPlaceholder = targetAccount.emailAddress == "[New Unconfigured Account]" && targetAccount.tokenKey.isEmpty
        func cancelSetup() {
            print("\n Cancelled.")
            // Deliberately NOT calling keyboard.disableRawMode() here — this function returns
            // control back into showAccountMatrix()'s still-running raw-mode loop (it's a direct
            // function call, not a screen-stack transition), so raw mode needs to stay enabled
            // for that loop's next keyboard.readKey() to work correctly. Disabling it here was
            // the actual bug: it broke ESC (and every other key) in the account list right
            // after cancelling out of this screen, since nothing re-enabled it afterward.
            if isUnconfiguredPlaceholder {
                dataLock.lock()
                if accountIndex < accounts.count { accounts.remove(at: accountIndex) }
                saveConfiguration()
                dataLock.unlock()
            }
        }
        ...
        fflush(stdout)
        guard let providerRaw = keyboard.readCanonicalLineOrEscape() else { cancelSetup(); return }
        let providerInput = providerRaw.trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        if targetAccount.type == .custom {
            ...
            fflush(stdout)
            guard let imapHostRaw = keyboard.readCanonicalLineOrEscape() else { cancelSetup(); return }
            let imapHostInput = imapHostRaw.trimmingCharacters(in: .whitespacesAndNewlines)
            ...
            fflush(stdout)
            guard let imapPortRaw = keyboard.readCanonicalLineOrEscape() else { cancelSetup(); return }
            let imapPortInput = imapPortRaw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let port = Int(imapPortInput) { targetAccount.imapPort = port }
            
            print(" SMTP Host [\(targetAccount.smtpHost)]: ", terminator: "")
            fflush(stdout)
            guard let smtpHostRaw = keyboard.readCanonicalLineOrEscape() else { cancelSetup(); return }
            let smtpHostInput = smtpHostRaw.trimmingCharacters(in: .whitespacesAndNewlines)
            ...
            fflush(stdout)
            guard let smtpPortRaw = keyboard.readCanonicalLineOrEscape() else { cancelSetup(); return }
            let smtpPortInput = smtpPortRaw.trimmingCharacters(in: .whitespacesAndNewlines)
            ...
            if let idx = targetAccount.folders.firstIndex(where: { $0.name == "SENT" }) {
                ...
                fflush(stdout)
                guard let vRaw = keyboard.readCanonicalLineOrEscape() else { cancelSetup(); return }
                let v = vRaw.trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        fflush(stdout) 
        guard let emailRaw = keyboard.readCanonicalLineOrEscape() else { cancelSetup(); return }
        let emailInput = emailRaw.trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        fflush(stdout) 
        guard let tokenRaw = keyboard.readMaskedLine() else { cancelSetup(); return }
        let tokenInput = tokenRaw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !tokenInput.isEmpty {
            targetAccount.tokenKey = LocalCryptoEngine.encrypt(tokenInput)
        }
        
        // Auto-discover real folder names for known providers — the actual fix for Yahoo (and
        // any other provider whose real folder names don't match the hardcoded per-type guess
        // set when the account was first created). Skipped for .custom, since that flow above
        // already let the person manually enter folder paths on purpose — discovery shouldn't
        // silently override something they just typed. See IMAPSession.discoverSpecialUseFolders().
        if targetAccount.type != .custom, !targetAccount.tokenKey.isEmpty {
            print("\n Verifying connection and discovering real folder names...")
            fflush(stdout)
            let password = LocalCryptoEngine.decrypt(targetAccount.tokenKey)
            if let session = IMAPSession(host: targetAccount.imapHost, port: UInt16(clamping: targetAccount.imapPort)),
               session.login(user: targetAccount.emailAddress, password: password) {
                MailDebugLogger.log("Folder discovery: logged in OK to \(targetAccount.imapHost) as \(targetAccount.emailAddress)", category: "FOLDER-DISCOVERY")

                var discovered = session.discoverSpecialUseFolders()
                if discovered.isEmpty {
                    discovered = session.discoverFoldersByHeuristic()
                }
                session.logout()

                if let sentPath = discovered["\\Sent"], let idx = targetAccount.folders.firstIndex(where: { $0.name == "SENT" }) {
                    targetAccount.folders[idx].serverPath = sentPath
                }
                if let junkPath = discovered["\\Junk"], let idx = targetAccount.folders.firstIndex(where: { $0.name == "JUNK" }) {
                    targetAccount.folders[idx].serverPath = junkPath
                }
                if let trashPath = discovered["\\Trash"], let idx = targetAccount.folders.firstIndex(where: { $0.name == "TRASH" }) {
                    targetAccount.folders[idx].serverPath = trashPath
                }

                if discovered.isEmpty {
                    print(" \u{001B}[1;33mCouldn't discover real folder names — keeping the default guess (adjust manually later if sync fails).\u{001B}[0m")
                } else {
                    print(" \u{001B}[1;32mFolder names verified against the server.\u{001B}[0m")
                }
            } else {
                MailDebugLogger.log("Folder discovery: could not connect/login to \(targetAccount.imapHost) as \(targetAccount.emailAddress)", category: "FOLDER-DISCOVERY-ERR")
                print(" \u{001B}[1;33mCouldn't connect to verify folders — keeping the default guess. Check the App Secret Key if sync fails.\u{001B}[0m")
            }
        }
    ...
    func injectMockDataIfEmpty() {
        ...
        let primaryMock = EmailAccount(emailAddress: "first.last@email.com", type: .google)
