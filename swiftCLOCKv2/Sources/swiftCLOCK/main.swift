...
// a red second hand. Three complications replace the hour markers at 3/6/9:
// a compact date number at 3, tracking eyes (ported from swiftEYES) at 6,
// and a digital 24-hour readout at 9.
...
final class ClockFaceView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        drawDigital24Hour(center: center, radius: radius, hour24: hour24, minute: minute)
        if let window = self.window {
            drawEyes(center: center, radius: radius, window: window)
        }
    ...
    private func drawTicks(center: NSPoint, radius: CGFloat) {
        // Full symmetric ring — all 12 hour positions equal, including
        // 3/6/9. The date, digital readout, and eyes all sit much closer
        // to center (see their own radius fractions below), so there's
        // a clear radial gap and no collision with this outer ring.
        for i in 0..<60 {
    ...
    private func drawDate(center: NSPoint, radius: CGFloat, now: Date) {
        // Watch-style "SUN 09" date window at 3 o'clock, matching the
        // classic Day-Date convention — including the traditional red
        // Sunday, a detail found on real watches that popularized this
        // exact layout. All-caps day abbreviation to match that same
        // watch-dial convention (Day-Date models print the day in full
        // caps, not mixed case).
        ...
        let dayAbbrev = weekdayAbbreviations[(weekday - 1) % 7]
        let text = String(format: "%@ %02d", dayAbbrev, day)
        let color = (weekday == 1) ? NSColor.systemRed : markColor
        let font = NSFont.systemFont(ofSize: radius * 0.11, weight: .medium)   // matches drawDigital24Hour's size exactly
        let labelPoint = point(center: center, radius: radius * 0.54, angle: .pi / 2)   // matches drawDigital24Hour's radius for left/right balance
        drawCenteredText(text, at: labelPoint, font: font, color: color)
    }

    private func drawDigital24Hour(center: NSPoint, radius: CGFloat, hour24: Int, minute: Int) {
        // Plain digital 24-hour readout at 9 o'clock.
        let text = String(format: "%02d:%02d", hour24, minute)
        let font = NSFont.systemFont(ofSize: radius * 0.11, weight: .medium)   // matches drawDate's font exactly, was monospacedDigitSystemFont which visibly didn't match
        let labelPoint = point(center: center, radius: radius * 0.60, angle: 3 * .pi / 2)   // small additional inward nudge from 0.62
        drawCenteredText(text, at: labelPoint, font: font, color: markColor)
    }

    private func drawEyes(center: NSPoint, radius: CGFloat, window: NSWindow) {
        // Tracking eyes just above 6 o'clock — the same spot the date used
        // to occupy. Ported directly from swiftEYES' own pupil-tracking
        // math, just fixed at a pair rather than swiftEYES' configurable
        // row of however-many.
        let eyesCenter = point(center: center, radius: radius * 0.570, angle: .pi)
        let eyeRadius = radius * 0.135
        let eyeSpacing = eyeRadius * 2.2

        let leftEyeRect = NSRect(
            x: eyesCenter.x - eyeSpacing / 2 - eyeRadius, y: eyesCenter.y - eyeRadius,
        ...
        drawTrackingEye(eyeRect: leftEyeRect, mouseScreen: mouseScreen, window: window)
        drawTrackingEye(eyeRect: rightEyeRect, mouseScreen: mouseScreen, window: window)
    }

    private func drawTrackingEye(eyeRect: NSRect, mouseScreen: NSPoint, window: NSWindow) {
        ...
        // Convert this eye's center to screen coordinates, same technique
        // swiftEYES uses, so the pupil can track the real global cursor
        // position rather than just mouse events local to this view.
        ...
        let pupilTravelRatio: CGFloat = 0.55
        let pupilRadiusRatio: CGFloat = 0.28
