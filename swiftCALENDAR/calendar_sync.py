...
def parse_ics_date(raw_line):
    ...
    if len(date_str) == 8 and date_str.isdigit():
        # All-day event (VALUE=DATE, no time/timezone) — meant to represent a calendar date, not
        # a specific moment. Using midnight UTC here was a real bug: displayed in any negative-
        # UTC-offset local timezone (US Eastern, Central, etc.), midnight UTC rolls back to the
        # evening of the PREVIOUS day locally, shifting every all-day event a day earlier than
        # it should be. Noon UTC is the standard fix for this class of bug — it never crosses a
        # day boundary for any realistic timezone offset, same pattern already used for TAF's
        # "tomorrow" date elsewhere in this file.
        return f"{date_str[:4]}-{date_str[4:6]}-{date_str[6:8]}T12:00:00Z", True
