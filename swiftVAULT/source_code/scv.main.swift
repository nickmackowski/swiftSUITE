...
class VaultDebugLogger {
    ...
    /// beside the binary). Created on first access if it doesn't exist yet. vault.json and
    ...
    static let logURL: URL = {
        ...
        return logsDir.appendingPathComponent("swiftvault_\(ts).log")
    }()
...
class PasswordVaultManager {
    ...
    private func printStandardHeader() {
        ...
        let innerWidth = 118
        let titleText = "swiftVAULT v3.01.07.29c"  // plain for layout; colored below
    ...
    func showResultsScreen(results: [Credential], title: String) {
        ...
        while true {
            ...
            case .other(let ch):
                // Nav footer — was missing entirely from this screen; every other app already
                // has this on its own search-results screen. [V] is current app (no-op).
                let lower = Character(ch.lowercased())
                let navMap: [Character: String] = [
                    "t": "swiftCONTACTS", "c": "swiftCALENDAR",
                    "m": "swiftMAIL",     "n": "swiftNOTES",
        ...
        while true {
            ...
            case .other(let ch):
                ...
                if lower == "v" {
                    ...
                    // Nav footer keys. Missing disableRawMode()/enableRawMode() bracketing here
                    // before (bug) — every other navigateToApp() call site, including
                    // showWorkspace() right in this file, wraps the call that way so the target
                    // app doesn't inherit this app's raw terminal mode.
                    let navMap: [Character: String] = [
                        "t": "swiftCONTACTS", "c": "swiftCALENDAR",
                        "m": "swiftMAIL",     "n": "swiftNOTES",
                        "s": "swiftSTOCKS",
