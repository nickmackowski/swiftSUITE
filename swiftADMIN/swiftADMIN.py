...
def header(subtitle: str = ""):
    ...
    # orange already used for the trailing "c" version-credit suffix across the
    # Swift apps) -- reversed from the old whole-string bold-cyan title, matching
    # the suite-wide "swift stays white, app name gets one accent color" scheme.
    title = Text("swift", style="white")
    title.append("ADMIN", style="bold color(208)")
    title.append(f" v{VERSION}", style="white")
    if subtitle:
...
def discover_projects():
    ...
    app root):
      - NEW layout: source lives in <app>/source_code/*.main.swift
      - OLD layout: source lives directly in <app>/*.main.swift (flat)
    Detected per-app purely from whether a source_code/ subfolder exists —
    no separate "which apps are migrated" tracking needed. build_one() needs
    no changes for this: the returned source path already includes the
    "source_code/" prefix when applicable, and since it builds with cwd set
    to the app root, the compiled binary always lands at the app root either
    way — only the source path differs.
    """
    discovered = []
    try:
    ...
    for entry in entries:
        ...
        try:
            if source_code_dir.is_dir():
                swift_sources = sorted(f for f in os.listdir(source_code_dir) if f.endswith(".main.swift"))
...
def build_single():
    ...
    for part in raw_parts:
        if part.isdigit() and 1 <= int(part) <= len(projects):
            idx = int(part)
            if idx not in seen_indices:  # silently dedupe if the same number was listed twice
